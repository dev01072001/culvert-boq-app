...
# --- 1. THE LOOKUP TABLE ---
# # Explained: Standard dimensions (A to G) for Independent Retaining Walls based on Height 'H'.
lookup_data = {
...
def calculate_geometry_pipeline(row):
    ...
    # # Explained: Independent Wall Height is fixed from Invert to Top of Slab.
    if sel == "Independent Retaining wall":
        row["PROT_HT"] = VC + TS
        row["PROT_LEN"] = (1.5 * row["PROT_HT"]) - SW
    
    # # Explained: Wing Walls use average height and 45-degree spread length.
...
def assign_protection_sections(row):
    # # Explained: Rounds height to nearest integer to find A-G table row.
...
def calculate_scour_geometry(row):
    # # Explained: Calculates Curtain Wall length and Apron Plan Area for scour protection.
...
def calculate_master_quantities(row):
    ...
    # # Explained: Total Reinforcement = (Box concrete * Ratio) + (Prot concrete * Ratio).    
    row["TOTAL_STEEL_KG"] = (row["TOTAL_RCC_BOX_M35"]*row["PERCENT_STEEL_BOX"]) + (row["TOTAL_M35_PROTECTION"]*row["PERCENT_STEEL_PROT"])
    h_f = VC+TS+BS
    # # Explained: Backfill = 1:1 pressure wedge. Filter Media = 600mm thk layer.
    ...
    # # Explained: Aprons: 300mm Rigid (0.3), 450mm Launching (0.45).
    ...
    # # Explained: Weep holes count at 1m spacing if height > 0.6m.
...
if st.button("🚀 Calculate Final BOQ"):
    ...
    # # Explained: Geometry Setup before Engine run.
    ...
    # # FULL RESULTS LIST
    results = [
        ["Barrel Length", f"{row['BARREL_LENGTH']:.3f}", "m", "Width TCS + (Slopes * Ratio * Cushion)"],
        ["Total Excavation", f"{row['TOTAL_EXCAVATION']:.2f}", "m3", "1.0m offset pay-lines for box & walls"],
        ["PCC Box Base", f"{row['QTY_PCC_BOX']:.2f}", "m3", "100mm layer + 300mm offset"],
        ["PCC Prot Base", f"{row['QTY_PCC_PROT']:.2f}", "m3", "100mm layer under protection footings"],
        ["RCC M35 Box", f"{row['TOTAL_RCC_BOX_M35']:.2f}", "m3", "Slabs + Walls + Haunches + Shear Keys"],
        ["RCC M35 Protection", f"{row['TOTAL_M35_PROTECTION']:.2f}", "m3", "Stem and Footing volume"],
        ["Total Backfill", f"{row['TOTAL_BACKFILL']:.2f}", "m3", "1:1 earth pressure wedge behind walls"],
        ["Filter Media", f"{row['TOTAL_FILTER_MEDIA']:.2f}", "m3", "600mm thk vertical drainage layer"],
        ["Wearing Course", f"{row['QTY_WC']:.2f}", "m3", "150mm thick road crust layer"],
        ["Rigid Apron", f"{row['QTY_RIGID_APRON_M3']:.2f}", "m3", "300mm thick PCC/RCC floor"],
        ["Launching Apron", f"{row['QTY_LAUNCHING_APRON_M3']:.2f}", "m3", "450mm thick flexible stone protection"],
        ["Total Steel", f"{row['TOTAL_STEEL_KG']:.2f}", "kg", "Concrete volume * Steel Ratios"],
        ["Weep Holes", f"{row['TOTAL_WEEP_HOLES_NOS']:.0f}", "Nos", "PVC pipes spaced at 1m c/c"]
    ]
    
    df_boq = pd.DataFrame(results, columns=["Description", "Quantity", "Unit", "Explanation"])
    
    # # CSS Fix for Small Font Table
    st.markdown("""<style> table { font-size: 11px !important; } th { background-color: #2e7d32 !important; color: white !important; } </style>""", unsafe_content_type=True)
    st.table(df_boq)
    ...
    for index, r in df_boq.iterrows():
        pdf.cell(50, 8, r['Description'], border=1)
        pdf.cell(25, 8, r['Quantity'], border=1, align='C')
        pdf.cell(15, 8, r['Unit'], border=1, align='C')
        pdf.cell(100, 8, r['Explanation'], border=1)
        pdf.ln()
