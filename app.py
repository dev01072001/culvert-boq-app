...
def run_engine(row):
    ...
    row["R_M25_TOE"], row["R_M15_CURT"] = row["TOE_X_A"]*row["L_TOE"]*row["TOE_COUNT"]*n, row["C_A_T"]*n
...
with c2:
    cl, ln, vc = st.number_input("No. of Cells", value=1), st.number_input("Span L", value=2.0), st.number_input("VC Inner", value=2.0)
...
if st.button("🚀 Generate Final BOQ"):
    ...
    st.success("✅ Categorised Results:")
    table = [
        ["Excavation (Box)", f"{res['E_BOX']:.2f}", "m3", "1.0m offset"],
        ["Excavation (Prot)", f"{res['E_PROT']:.2f}", "m3", "Base + offset"],
        ["Excavation (Return)", f"{res['E_RET']:.2f}", "m3", "3.1x2.5 fixed"],
        ["Excavation (Scour)", f"{res['E_SCOUR']:.2f}", "m3", "Footprint"],
        ["**TOTAL EXCAVATION**", f"{res['GRAND_EXC']:.2f}", "m3", ""],
        ["PCC (Box Foundation)", f"{res['P_BOX']:.2f}", "m3", "300mm offset"],
        ["PCC (Prot Foundation)", f"{res['P_PROT']:.2f}", "m3", "300mm offset"],
        ["PCC (Return Foundation)", f"{res['P_RET']:.2f}", "m3", "Fixed"],
        ["PCC (Scour Components)", f"{res['P_SCOUR']:.2f}", "m3", "Base"],
        ["**TOTAL PCC M15**", f"{res['GRAND_PCC']:.2f}", "m3", ""],
        ["RCC M35 (Box)", f"{res['R_BOX']:.2f}", "m3", "Main Barrel"],
        ["RCC M35 (Prot)", f"{res['R_PROT']:.2f}", "m3", "Main Stems"],
        ["RCC M35 (Return)", f"{res['R_RET']:.2f}", "m3", "1.0m stem ht"],
        ["RCC M25 (Toe Walls)", f"{res['R_M25_TOE']:.2f}", "m3", "0.370 area"],
        ["RCC M15 (Curtain Walls)", f"{res['R_M15_CURT']:.2f}", "m3", "0.7m width"],
        ["Steel (Total)", f"{res['S_BOX']+res['S_PROT']:.2f}", "kg", "Ratios applied"],
        ["Backfill (1:1)", f"{res['BF']:.2f}", "m3", "0.5*H^2*L"],
        ["Filter Media", f"{res['FM']:.2f}", "m3", "600mm layer"],
        ["Weep Holes (100mm)", f"{res['WEEP']:.0f}", "Nos", "Col x Row x Walls"]
    ]
    ...
    st.download_button("📥 Download PDF", data=pdf.output(dest='S').encode('latin-1'), file_name="BOQ.pdf", mime="application/pdf", key="dl")
