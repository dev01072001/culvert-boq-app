...
with col2:
    cells, L, VC, h_size = st.number_input("Cells", min_value=1, value=1), st.number_input("Span L", value=2.0), st.number_input("VC", value=2.0), st.number_input("Haunch", value=0.15)
with col3:
...
if st.button("🚀 Calculate BOQ"):
    row = {
        ...
        "SIDE_WALL_THICKNESS": t_side, "MID_WALL_THICKNESS": 0.0, "PERCENT_STEEL_BOX": st_box, "PERCENT_STEEL_PROT": st_prot
    }
    row["OUTER_WIDTH"] = (cells * L) + (2 * t_side)
    ...
    results = [
        ["Barrel Length", f"{row['BARREL_LENGTH']:.3f}", "m", "TCS + (Slopes*Ratio*Cushion)"],
        ...
        ["PCC M15 (Prot Base)", f"{row['QTY_PCC_PROT']:.2f}", "m3", "100mm thk under footings"],
        ["RCC M35 Box", f"{row['TOTAL_RCC_BOX_M35']:.2f}", "m3", "Slabs + Walls + Haunches + Keys"],
        ["RCC M35 Protection", f"{row['TOTAL_M35_PROTECTION']:.2f}", "m3", "Stem and Footing volume"],
        ["Total Backfill (1:1)", f"{row['TOTAL_BACKFILL']:.2f}", "m3", "1:1 earth pressure wedge"],
        ["Filter Media", f"{row['TOTAL_FILTER_MEDIA']:.2f}", "m3", "600mm thk vertical drainage"],
        ["Wearing Course", f"{row['QTY_WC']:.2f}", "m3", "150mm road crust"],
        ["Rigid Apron", f"{row['QTY_RIGID_APRON_M3']:.2f}", "m3", "300mm floor protection"],
        ["Launching Apron", f"{row['QTY_LAUNCHING_APRON_M3']:.2f}", "m3", "450mm stone protection"],
        ["Total Steel", f"{row['TOTAL_STEEL_KG']:.2f}", "kg", "Concrete volume * Ratio"],
        ["Weep Holes", f"{row['TOTAL_WEEP_HOLES_NOS']:.0f}", "Nos", "1m c/c spacing"]
    ]
    ...
    pdf = FPDF(); pdf.add_page(); pdf.set_font("Arial", 'B', 12); pdf.cell(190, 10, "BRIDGE CULVERT BOQ REPORT", ln=True, align='C')
    pdf_out = pdf.output(dest='S').encode('latin-1')
    st.download_button("📥 Download PDF", data=pdf_out, file_name="Culvert_BOQ.pdf")
