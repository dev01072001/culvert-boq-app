...
def run_master_logic(row):
    ...
    pw = row["PROT_AVG_WIDTH"] if "U-Trough" in sel else row["W"]
    ...
    if sel in ["Independent Retaining wall", "Wing Wall + Return Wall"]:
        ...
        area_s = ((row["B"] + 0.3)/2) * row["PROT_HT"]
    ...
    # Finishing (FM, BF, Steel, Pitching)
...
with c1:
    rw, frl, gl = st.number_input("TCS Width (m)", value=7.0), st.number_input("FRL", value=5.0), st.number_input("GL", value=2.0)
...
if st.button("🚀 Generate Final BOQ"):
    ...
    res = run_master_logic(data)
    st.success("✅ BOQ Generated.")

    rows = [
        ["Excavation (Box)", f"{res['EXC_BOX']:.2f}", "m3", "1.0m offset"],
        ["Excavation (Prot)", f"{res['EXC_PROT']:.2f}", "m3", "Footing logic"],
        ["Excavation (Return)", f"{res['EXC_RET']:.2f}", "m3", "Fixed 3.1x2.5"],
        ["Excavation (Toe/Curt)", f"{res['EXC_TOE_CURT']:.2f}", "m3", "Structural footprint"],
        ["**TOTAL EXCAVATION**", f"{res['GRAND_EXC']:.2f}", "m3", "Sum of earthwork"],
        ["PCC M15 (Grand Total)", f"{res['PCC_TOT']:.2f}", "m3", "100mm layer"],
        ["RCC M35 (Box Structure)", f"{res['RCC_BOX']:.2f}", "m3", "Box structural concrete"],
        ["RCC M35 (Prot + Return)", f"{res['RCC_PROT']:.2f}", "m3", "Wall stems and footings"],
        ["RCC M25 (Toe Walls)", f"{res['RCC_M25_TOE']:.2f}", "m3", "0.370 area"],
        ["RCC M15 (Curtain Walls)", f"{res['RCC_M15_CURT']:.2f}", "m3", "0.7m width"],
        ["Total Steel Reinforcement", f"{res['STEEL_TOT']:.2f}", "kg", "Ratios applied"],
        ["Total Backfill (1:1)", f"{res['BF_TOT']:.2f}", "m3", "0.5*H^2*L Rule"],
        ["Total Filter Media", f"{res['FM_TOT']:.2f}", "m3", "600mm layer"],
        ["Wearing Course", f"{res['WC_TOT']:.2f}", "m3", "150mm PCC"],
        ["Rigid Apron", f"{res['RIGID']:.2f}", "m3", "300mm floor"],
        ["Launching Apron", f"{res['LAUNCH']:.2f}", "m3", "450mm thk"],
        ["Quadrant Pitching", f"{res['PITCH']:.2f}", "m3", "Quadrant slant area"],
        ["Weep Holes (100mm)", f"{res['WEEP_N']:.0f}", "Nos", "1m spacing"]
    ]
    st.table(pd.DataFrame(rows, columns=["Description", "Quantity", "Unit", "Logic Audit"]))
